...
class GetTaintSink:
    ...
    def __init__(self, top_xref_limit = 50, min_arg_cnt = 2, max_arg_cnt = 3, interfunction_level = 1, max_bb_size = 8, max_call_count = 3):
        ...
        self.taint_sinks = []
        self.top_xref_limit = len(helper.functions_info)
    ...
    def is_memcpy_block(self, basic_block, external_calls, func_start_addr, func_end_addr, interfunction_level = 0):
        ...
        bb_start = get_name(basic_block.start_ea)
        #print("0x%x-0x%x" % (basic_block.start_ea,basic_block.end_ea))
        while instr_addr < basic_block.end_ea:
            ...
            if mnemonics in branch_mnemonics:
                branch_tgt = idc.print_operand(instr_addr, 0)
                branch_tgt_addr = idc.get_operand_value(instr_addr, 0)
                if branch_tgt in registers:
                    instr_addr = idc.next_head(instr_addr)
                    continue
                #print(opcode_type)
                #print(bb_start)
                #print("branch_tgt:", branch_tgt)
    ...
    def get_sinks(self):
        ...
        for function_info in self.memcpy_like_functions:
            ...
            if self.is_memcpy_function(function_start_addr):
                fname = get_func_name(function_start_addr)
                self.taint_sinks.append((function_start_addr, fname))
    def ret_taint_sinks(self):
...
if __name__ == "__main__":
    ...
    taint_sinks = get_taint_sink.ret_taint_sinks()
    ...
    print (taint_sinks)
    end_stime = time.time()
    for func in taint_sinks:
        func_ea = func[0]
        func_name = func[1].strip("_")
    ...
    #ea = get_screen_ea()
    #print(get_taint_sink.is_memcpy_function(ea))
